...
public class IVGJPS extends JumpPointSearch {
    ...
    public IVGJPS(GridGraph graph, int sx, int sy, int ex, int ey, float existingPathLength) {
     ...
     */
    protected final boolean outsideEllipse(float parentDistance, int parX, int parY, int x, int y) {
    ...
    protected final boolean relax(int u, int v, float weightUV, int currX, int currY) {
    ...
    public final float heuristicValue(int x, int y) {
        int index = graph.toOneDimIndex(x, y);
        return distance(index)/APPROXIMATION_RATIO;
    }

    
    protected final int jump(float parentDistance, int x, int y, int dx, int dy) {
        if (dx < 0) {
            if (dy < 0) {
    ...
    private final void maybeUpdateDistance(float parentDistance, int parX, int parY, int x, int y) {
