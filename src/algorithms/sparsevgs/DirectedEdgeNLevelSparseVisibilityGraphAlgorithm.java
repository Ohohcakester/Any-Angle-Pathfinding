...
public class DirectedEdgeNLevelSparseVisibilityGraphAlgorithm extends AStarStaticMemory {
    private DirectedEdgeNLevelSparseVisibilityGraph visibilityGraph;
    private boolean reuseGraph = false;
    private int levelLimit = Integer.MAX_VALUE;
    
    private DirectedEdgeNLevelSparseVisibilityGraphAlgorithm(GridGraph graph, int sx, int sy, int ex, int ey) {
        super(graph, sx, sy, ex, ey);
    }
    
    public static DirectedEdgeNLevelSparseVisibilityGraphAlgorithm graphReuse(GridGraph graph, int sx, int sy, int ex, int ey) {
        DirectedEdgeNLevelSparseVisibilityGraphAlgorithm algo = new DirectedEdgeNLevelSparseVisibilityGraphAlgorithm(graph, sx, sy, ex, ey);
    ...
    public DirectedEdgeNLevelSparseVisibilityGraph getVisibilityGraph() {
...
TimeCounter.counterA++;
            ...
            float[] outgoingEdgeWeights = visibilityGraph.outgoingEdgeWeightss[current];
            int[] isMarkedIndexes = visibilityGraph.outgoingEdgeIsMarkedIndexess[current];
            
            for (int i=0;i<nOutgoingEdges;++i) {
                int dest = outgoingEdges[i];
                float weight = outgoingEdgeWeights[i];
                if (visibilityGraph.isMarked[isMarkedIndexes[i]] &&
                        !Memory.visited(dest) &&
...
_ed = System.nanoTime();
TimeCounter.timeC += (_ed-_st);

//_st = System.nanoTime();
        resolveFinalPath();
//_ed = System.nanoTime();
//TimeCounter.timeD += (_ed-_st);
    }

    protected void setupVisibilityGraph() {
        if (reuseGraph) {
            visibilityGraph = DirectedEdgeNLevelSparseVisibilityGraph.initialiseNew(graph, levelLimit);
        } else {
    ...
    private void resolveFinalPath() {
        ...
        while (current != -1) {
            if (current < -1) {
                ...
                int nOutgoingEdges = visibilityGraph.nOutgoingEdgess[current];
                int[] outgoingEdges = visibilityGraph.outgoingEdgess[current];
                int[] outgoingEdgeLevels = visibilityGraph.outgoingEdgeLevelss[current];
                int[] oppositeIndexes = visibilityGraph.outgoingEdgeOppositeIndexess[current];
                
                boolean done = false;
                for (int i=0;i<nOutgoingEdges;++i) {
                    int next = outgoingEdges[i];
                    if (outgoingEdgeLevels[i] != DirectedEdgeNLevelSparseVisibilityGraph.LEVEL_W ||
                        visibilityGraph.outgoingEdgeLevelss[next][oppositeIndexes[i]] != DirectedEdgeNLevelSparseVisibilityGraph.LEVEL_W ||
    ...
    protected void saveVisibilityGraphSnapshot(boolean showMarked) {
        final int LEVEL_W = DirectedEdgeNLevelSparseVisibilityGraph.LEVEL_W;
        ...
        for (int i=0;i<size;i++) {
            ...
            int[] outgoingEdgeLevels = visibilityGraph.outgoingEdgeLevelss[i];
            int[] isMarkedIndexes = visibilityGraph.outgoingEdgeIsMarkedIndexess[i];
            int[] oppositeIndex = visibilityGraph.outgoingEdgeOppositeIndexess[i];
            ...
            for (int j=0;j<nOutgoingEdges;++j) {
                ...
                boolean isMarked = visibilityGraph.isMarked[isMarkedIndexes[j]];
                int edgeLevel = outgoingEdgeLevels[j];
                if (oppositeIndex[j] != -1) {
                    int opposite = oppositeIndex[j];
                    int oppositeLevel = visibilityGraph.outgoingEdgeLevelss[neighbour][opposite];
                    if (oppositeLevel < edgeLevel) edgeLevel = oppositeLevel;
                    
                    isMarked |= visibilityGraph.isMarked[visibilityGraph.outgoingEdgeIsMarkedIndexess[neighbour][opposite]];
                }
                        
                int colourIndex = Math.min(edgeLevel, vertexColours.length-1);
                Color color = vertexColours[colourIndex];
                if (showMarked && isMarked) color = Color.BLACK;
                else if (edgeLevel == LEVEL_W) color = levelWEdgeColour;
                
                SnapshotItem snapshotItem = SnapshotItem.generate(path, color);
                if (colourIndex != 1 || color == Color.BLACK)
            ...
            if (visibilityGraph.nSkipEdgess[i] >= 3) {
                // Regular vertex.
                snapshotItem = SnapshotItem.generate(vert, Color.WHITE);
            }
            else if (visibilityGraph.nSkipEdgess[i] > 0)  {
                snapshotItem = SnapshotItem.generate(vert, Color.RED);
            }
            else {
    ...
    private static final Color levelWEdgeColour = new Color(127,191,255);
    ...
    private final String tostr(int v) {
        return visibilityGraph.xPositions[v] + "," + visibilityGraph.yPositions[v];
    }
}
