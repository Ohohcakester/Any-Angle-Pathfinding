...
public class DirectedEdgeNLevelSparseVisibilityGraphAlgorithm extends AStarStaticMemory {
    ...
    public void computePath() {
        ...
        {
            LineOfSightScanner losScanner = visibilityGraph.findReachableVerticesFromStart(sx, sy);
            int nSuccessors = losScanner.nSuccessors;
            int[] successorsX = losScanner.successorsX;
            int[] successorsY = losScanner.successorsY;
            for (int i=0;i<nSuccessors;++i) {
                int destX = successorsX[i];
                int destY = successorsY[i];
                int dest = visibilityGraph.nodeIndex[destY][destX];
        ...
        while (!pq.isEmpty()) {
            ...
            if (!visibilityGraph.cameFromLevelWEdge[current] || visibilityGraph.nSkipEdgess[current] == 0) {
                ...
                for (int i=0;i<nDirectedNeighbours;++i) {
                    int index = directedNeighbourOutgoingIndexes[i];
            ...
            // Scan through level-W edges to neighbours (ONLY IF WE ARENT ON A SKIP VERTEX)
            ...
            // Scan through marked edges to neighbours (includes marked level-W edges)
            {
            ...
            // Check if there is an edge to the goal.
    ...
    /**
     * Idea: If there are no possible good exits from the current vertex,
     *       don't even bother putting it in the priority queue.
     *       This check is to be done before adding to the PQ.
     * Intention: Prevents irrelevant high-f-value items from flooding the priority queue.
     *            Each newly-added lower-valued item has to bubble through these useless high-f-valued items,
     *            which slows down the priority queue.   
     * 
     * Problem: The currently-being-used condition is always true as we always have directed neighbours going out.
     *          It's just that those directed neighbours are not taut exits. But we can't easily detect that.
     */
    /*private final boolean isUseful(int vertex) {
        boolean useful = visibilityGraph.nDirectedNeighbourss[vertex] +
                visibilityGraph.nMarkedEdgess[vertex] +
                visibilityGraph.nSkipEdgess[vertex] +
                visibilityGraph.nLevelWNeighbourss[vertex] > 0;
    }*/

    protected void setupVisibilityGraph() {
        if (reuseGraph) {
            visibilityGraph = DirectedEdgeNLevelSparseVisibilityGraph.initialiseNew(graph);
        } else {
    ...
    protected void saveVisibilityGraphSnapshot(boolean showMarked) {
        ...
        for (int i=0;i<size;i++) {
            ...
            for (int j=0;j<nOutgoingEdges;++j) {
                ...
                int colourIndex = 1;//Math.min(visibilityGraph.edgeLevels[edgeIndex], vertexColours.length-1);
