...
public class VisibilityGraphAlgorithmOptimised extends AStarStaticMemory {
    protected VisibilityGraphOptimised visibilityGraph;
    protected boolean reuseGraph = false;
    
    private VisibilityGraphAlgorithmOptimised(GridGraph graph, int sx, int sy, int ex, int ey) {
        super(graph, sx, sy, ex, ey);
    }
    
    public static VisibilityGraphAlgorithmOptimised graphReuse(GridGraph graph, int sx, int sy, int ex, int ey) {
        VisibilityGraphAlgorithmOptimised algo = new VisibilityGraphAlgorithmOptimised(graph, sx, sy, ex, ey);
        algo.reuseGraph = true;
        return algo;
    }

    public VisibilityGraphOptimised getVisibilityGraph() {
...
TimeCounter.counterA++;
            ...
            VGNode node = visibilityGraph.getOutgoingEdges(current);
    ...
    protected void setupVisibilityGraph() {
        if (reuseGraph) {
            visibilityGraph = VisibilityGraphOptimised.getStoredGraph(graph);
        } else {
    ...
    protected void saveVisibilityGraphSnapshot() {
        ...
        for (int i=0;i<size;i++) {
            VGNode node = visibilityGraph.getOutgoingEdges(i);
            int[] outgoingEdges = node.outgoingEdges;
            int nEdges = node.nEdges;
