...
public class DirectedEdgeNLevelSparseVisibilityGraph {
    ...
    public int[][] outgoingEdgeLevelss;
    public int[][] outgoingEdgeOppositeIndexess;
    public float[][] outgoingEdgeWeightss;
    public int[][] outgoingEdgeIsMarkedIndexess;
    ...
    public int[][] outgoingSkipEdgeNextNodeOutgoingIndex; // value: edgeIndex of edge to next node in the actual path.
    ...
    */
    ...
    public static final DirectedEdgeNLevelSparseVisibilityGraph initialiseNew(GridGraph graph, int levelLimit) {
        if (DirectedEdgeNLevelSparseVisibilityGraph.storedGridGraph == graph && levelLimit == storedLevelLimit) {
long _s = System.nanoTime();
            storedVisibilityGraph.restoreOriginalGraph();
long _t = System.nanoTime();
TimeCounter.timeE += (_t-_s);
...
long _st = System.nanoTime();
        ...
        DirectedEdgeNLevelSparseVisibilityGraph.storedLevelLimit = levelLimit;
...
System.out.println("Construction Time: " + (_ed-_st)/1000000.);
    ...
    private final void constructGraph() {
        ...
        outgoingEdgeLevelss = new int[maxSize][];
        outgoingEdgeOppositeIndexess = new int[maxSize][];
        outgoingEdgeWeightss = new float[maxSize][];
        outgoingEdgeIsMarkedIndexess = new int[maxSize][];
        
        for (int i=0;i<maxSize;++i) {
            nOutgoingEdgess[i] = 0;
            outgoingEdgess[i] = new int[11];
            outgoingEdgeOppositeIndexess[i] = new int[11];
            outgoingEdgeIsMarkedIndexess[i] = new int[11];
        }
        for (int i=originalSize;i<maxSize;++i) {
            outgoingEdgeLevelss[i] = new int[11];
            outgoingEdgeWeightss[i] = new float[11];
        }
        ...
        outgoingSkipEdgeNextNodeOutgoingIndex = new int[maxSize][];
    ...
    private final void addEdge(int v1, int v2, float weight) {
        ...
        int v1Index;
        {
            ...
            if (v1Index >= outgoingEdgess[v1].length) {
                ...
                outgoingEdgeIsMarkedIndexess[v1] = Arrays.copyOf(outgoingEdgeIsMarkedIndexess[v1], newLength);
            }
            outgoingEdgess[v1][v1Index] = v2;
            outgoingEdgeIsMarkedIndexess[v1][v1Index] = isMarkedIndex;
            nOutgoingEdgess[v1]++;
        }
        ...
        int v2Index;
        {
            ...
            if (v2Index >= outgoingEdgess[v2].length) {
                ...
                outgoingEdgeIsMarkedIndexess[v2] = Arrays.copyOf(outgoingEdgeIsMarkedIndexess[v2], newLength);
            }
            outgoingEdgess[v2][v2Index] = v1;
            outgoingEdgeIsMarkedIndexess[v2][v2Index] = isMarkedIndex;
            nOutgoingEdgess[v2]++;
        }
    ...
    private final void initialiseEdgeProperties() {
        for (int i=0;i<nNodes;++i) {
            int nOutgoingEdges = nOutgoingEdgess[i];
            int maxOutgoingEdges = nOutgoingEdges + 2;

            outgoingEdgess[i] = Arrays.copyOf(outgoingEdgess[i], maxOutgoingEdges);
            outgoingEdgeOppositeIndexess[i] = Arrays.copyOf(outgoingEdgeOppositeIndexess[i], maxOutgoingEdges);
            outgoingEdgeIsMarkedIndexess[i] = Arrays.copyOf(outgoingEdgeIsMarkedIndexess[i], maxOutgoingEdges);
            
            outgoingEdgeLevelss[i] = new int[maxOutgoingEdges];
            Arrays.fill(outgoingEdgeLevelss[i], LEVEL_W);

            float[] weights = new float[maxOutgoingEdges];
            for (int j=0;j<nOutgoingEdges;++j) {
                int next = outgoingEdgess[i][j];
                float weight = graph.distance(xPositions[i],yPositions[i],xPositions[next],yPositions[next]);
                weights[j] = weight;
            }
            outgoingEdgeWeightss[i] = weights;
        }
    }
    ...
    private final void computeAllEdgeLevels() {
        ...
        while(hasChanged && currentLevel < levelLimit) {
            ...
            for (int v1=0;v1<nNodes;++v1) {
                ...
                int[] outgoingEdgeLevels = outgoingEdgeLevelss[v1];

                for (int j=0;j<nOutgoingEdges;++j) {
                    if (outgoingEdgeLevels[j] < currentLevel) continue;
                    ...
                    if (!hasTautEntrance(v2, v1, currentLevel)) {
                        outgoingEdgeLevels[j] = currentLevel;
                        hasChanged = true;
                    }
                }
            }
    ...
    private final boolean hasTautEntrance(int vFrom, int vTo, int currentLevel) {
        ...
        int[] oppositeIndexes = outgoingEdgeOppositeIndexess[vTo];
        for (int j=0;j<nOutgoingEdges;++j) {
            if (outgoingEdgeLevelss[outgoingEdges[j]][oppositeIndexes[j]] < currentLevel) continue;
    ...
    private final int markSkipVertices() {
        ...
        for (int i=0;i<nNodes;++i) {
            int nOutgoingEdges = nOutgoingEdgess[i];
            int[] outgoingEdgeLevels = outgoingEdgeLevelss[i];
            int[] outgoingEdges = outgoingEdgess[i];
            int[] oppositeIndexes = outgoingEdgeOppositeIndexess[i];

            int nLevelWNeighbours = 0;
            for (int j=0;j<nOutgoingEdges;++j) {
                if (outgoingEdgeLevels[j] == LEVEL_W && outgoingEdgeLevelss[outgoingEdges[j]][oppositeIndexes[j]] == LEVEL_W) ++nLevelWNeighbours;
            }
            ...
            int[] outgoingIndexes = new int[nLevelWNeighbours];
            float[] weights = new float[nLevelWNeighbours];
            int[] destinations = new int[nLevelWNeighbours];
            int current = 0;
            for (int j=0;j<nOutgoingEdges;++j) {
                if (outgoingEdgeLevels[j] == LEVEL_W && outgoingEdgeLevelss[outgoingEdges[j]][oppositeIndexes[j]] == LEVEL_W) {
                    nextNodes[current] = outgoingEdges[j];
                    outgoingIndexes[current] = j; 
                    current++;
                }
            }
            ...
            outgoingSkipEdgeNextNodeOutgoingIndex[i] = outgoingIndexes;
    ...
    private final void connectSkipEdgesAndGroupLevelWEdges() {
        for (int v1=0;v1<nNodes;++v1) {
            ...
            int[] nextNodeOutgoingIndexes = outgoingSkipEdgeNextNodeOutgoingIndex[v1];
            int[] outgoingSkipEdges = outgoingSkipEdgess[v1];
            float[] skipWeights = outgoingSkipEdgeWeightss[v1];

            float[] v1OutgoingEdgeWeights = outgoingEdgeWeightss[v1];
            int[] v1IsMarkedIndex = outgoingEdgeIsMarkedIndexess[v1];
            
            for (int j=0;j<nSkipEdges;++j) {
                // Start from vertex v1, move in direction j.
                int previous = v1;
                int current = nextNodes[j];
                int outgoingIndex = nextNodeOutgoingIndexes[j];
                outgoingSkipEdges[j] = current;
                skipWeights[j] = v1OutgoingEdgeWeights[outgoingIndex];
                int indexGroup = v1IsMarkedIndex[outgoingIndex];
                ...
                while (nSkipEdgess[current] == 0) {
                    ...
                    int[] outgoingEdgeLevels = outgoingEdgeLevelss[current];
                    int[] outgoingEdgeOppositeIndexes = outgoingEdgeOppositeIndexess[current];
                    float[] outgoingEdgeWeights = outgoingEdgeWeightss[current];

                    for (int k=0;k<nOutgoingEdges;++k) {
                        int next = outgoingEdges[k];
                        if (outgoingEdgeLevels[k] != LEVEL_W ||
                                outgoingEdgeLevelss[next][outgoingEdgeOppositeIndexes[k]] != LEVEL_W ||
                        ...
                        outgoingSkipEdges[j] = current;
                        skipWeights[j] += outgoingEdgeWeights[k];
                        outgoingEdgeIsMarkedIndexess[previous][k] = indexGroup;
                        break;
                    }
                }
    ...
    private final void groupAllLevelWEdgesTogether() {
        ...
        for (int i=0;i<nNodes;++i) {
            ...
            int[] outgoingEdgeLevels = outgoingEdgeLevelss[i];
            int[] oppositeIndexes = outgoingEdgeOppositeIndexess[i];
            int[] isMarkedIndex = outgoingEdgeIsMarkedIndexess[i];
            
            for (int j=0;j<nOutgoingEdges;++j) {
                if (outgoingEdgeLevels[j] == LEVEL_W && outgoingEdgeLevelss[outgoingEdges[j]][oppositeIndexes[j]] == LEVEL_W) {
                    if (markIndex == -1) markIndex = isMarkedIndex[j];
                    isMarkedIndex[j] = markIndex;
                }
            }
        }
    }
    ...
    private final void swapSkipEdges(int v, int i1, int i2) {
        ...
        {
            temp = outgoingSkipEdgeNextNodeOutgoingIndex[v][i1];
            outgoingSkipEdgeNextNodeOutgoingIndex[v][i1] = outgoingSkipEdgeNextNodeOutgoingIndex[v][i2];
            outgoingSkipEdgeNextNodeOutgoingIndex[v][i2] = temp;
        }
        {
...
TimeCounter.timeD += (_t-_s);
    ...
    private final void addTemporaryEdge(int v1, int v2, float weight) {
        ...
        if (index >= outgoingEdgess[v1].length) {
            ...
            outgoingEdgeIsMarkedIndexess[v1] = Arrays.copyOf(outgoingEdgeIsMarkedIndexess[v1], newLength);
            outgoingEdgeOppositeIndexess[v1] = Arrays.copyOf(outgoingEdgeOppositeIndexess[v1], newLength);
            outgoingEdgeLevelss[v1] = Arrays.copyOf(outgoingEdgeLevelss[v1], newLength);
            outgoingEdgeWeightss[v1] = Arrays.copyOf(outgoingEdgeWeightss[v1], newLength);
        }
        outgoingEdgess[v1][index] = v2;
        outgoingEdgeIsMarkedIndexess[v1][index] = edgeIndex;
        outgoingEdgeOppositeIndexess[v1][index] = -1;
        outgoingEdgeLevelss[v1][index] = 0;
        outgoingEdgeWeightss[v1][index] = weight;
    ...
    private final void markEdgesFrom(int source, boolean value) {
        queueSize = 0;
        {
            ...
            int[] isMarkedIndexes = outgoingEdgeIsMarkedIndexess[source];
            
            for (int i=0;i<nOutgoingEdges;++i) {
                int isMarkedIndex = isMarkedIndexes[i];
                if (isMarked[isMarkedIndex] == value) continue;
                isMarked[isMarkedIndex] = value;
        ...
        while (currIndex < queueSize) {
            ...
            int currLevel = outgoingEdgeLevelss[parent][parentOutgoingIndex];
            ...
            int[] outgoingEdgeLevels = outgoingEdgeLevelss[curr];
            int[] isMarkedIndexes = outgoingEdgeIsMarkedIndexess[curr];
            
            for (int i=0;i<nOutgoingEdges;++i) {
                int isMarkedIndex = isMarkedIndexes[i];
                if ((isMarked[isMarkedIndex] == value) || outgoingEdgeLevels[i] <= currLevel) continue;
