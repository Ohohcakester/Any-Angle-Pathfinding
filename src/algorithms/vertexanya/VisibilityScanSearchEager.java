...
public class VisibilityScanSearchEager extends PathFindingAlgorithm {
    ...
    public VisibilityScanSearchEager(GridGraph graph, int sx, int sy, int ex, int ey) {
    ...
    private void searchAllSuccessors() {
        int i = 0;
        while (i < numStates) {
            maybeSaveSearchSnapshot();
    ...
    private void addToOpen(int baseIndex, ScanInterval successor) {
        ...
        if (numStates >= states.length) {
            states = Arrays.copyOf(states, states.length*2);
        }
        states[numStates] = successor;
        ++numStates;
    }
    ...
    protected List<SnapshotItem> computeSearchSnapshot() {
        ArrayList<SnapshotItem> list = new ArrayList<>(numStates);

        for (int i=0;i<numStates;++i) {
            ScanInterval in = states[i];
