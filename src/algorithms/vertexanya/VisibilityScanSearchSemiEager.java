...
public class VisibilityScanSearchSemiEager extends PathFindingAlgorithm {
    private static final float EPSILON = 0.0001f;
    private static final float BASE_SLD_MULTIPLIER = 1.01f;
    private static final float MULTIPLIER = 1.01f;
    private static final double LOG_MULTIPLIER = Math.log(MULTIPLIER);
    
    // stateBuckets[k] contains intervals of fvalue < baseCutoff * MULTIPLIER^k   (+ EPSILON)
    // k = log(fvalue/baseCutoff) / LOG_MULTIPLIER;
    private float baseCutoff;
    private float currentCutoff;
    private float currentCutoffPlusEpsilon;
    
    private ScanInterval[][] stateBuckets; 
    private int[] bucketSizes;
    private int numBuckets;
    
    private int nextUnreusedBucket; // used for reusing buckets
    private int nextBucket;         // all buckets before nextBucket have been emptied.
    
    private int numQueueStates;
    private ScanInterval[] statesQueue; // states that are to be immediately popped and processed
    ...
    public VisibilityScanSearchSemiEager(GridGraph graph, int sx, int sy, int ex, int ey) {
    ...
    public void computePath() {
        ...
        baseCutoff = graph.distance(sx, sy, ex, ey) * BASE_SLD_MULTIPLIER;
        currentCutoff = baseCutoff;
        currentCutoffPlusEpsilon = baseCutoff + EPSILON;

        vertexPQ = new ReusableIndirectHeap(totalSize);
        stateBuckets = new ScanInterval[11][];
        bucketSizes = new int[11];
        statesQueue = new ScanInterval[11];
        numBuckets = 0;
        nextUnreusedBucket = 0;
        nextBucket = 0;
        ...
        while (nextBucket < numBuckets || !vertexPQ.isEmpty()) {

            while (vertexPQ.isEmpty() || currentCutoff < vertexPQ.getMinValue()) {
                currentCutoff *= MULTIPLIER;
                currentCutoffPlusEpsilon = currentCutoff + EPSILON;
                //System.out.println("Search " + nextBucket + "/" + numBuckets + " " + currentCutoff + " " + vertexPQ.getMinValue());
                if (nextBucket < numBuckets) {
                    searchStates(stateBuckets[nextBucket], bucketSizes[nextBucket]);
                } else if (vertexPQ.isEmpty()) {
                    return;
                }
                ++nextBucket;
            }
    ...
    private void searchStates(ScanInterval[] states, int size) {
        for (int i=0;i<size;++i) {
            maybeSaveSearchSnapshot();
            ScanInterval currState = states[i];
    ...
    private void clearStateQueue() {
        int i = 0;
        while (i < numQueueStates) {
            maybeSaveSearchSnapshot();
            ScanInterval currState = statesQueue[i];
            generateSuccessors(currState);
            ++i;
        }
        numQueueStates = 0;
    ...
    private void addToOpen(int baseIndex, ScanInterval successor) {
        ...
        if (fValue < currentCutoffPlusEpsilon) {
            // Put into queue for immediate processing.
            if (numQueueStates >= statesQueue.length) {
                statesQueue = Arrays.copyOf(statesQueue, statesQueue.length*2);
            }
            statesQueue[numQueueStates] = successor;
            ++numQueueStates;
        } else {
            // Can't immediately process. Put into bucket.
            int bucketNo = (int)(Math.log(fValue/baseCutoff) / LOG_MULTIPLIER);
            //System.out.println(fValue + " ==> bucket " + bucketNo); 
            
            // Construct any missing buckets leading up to the current bucket no.
            while (bucketNo >= numBuckets) {
                if (numBuckets >= bucketSizes.length) {
                    stateBuckets = Arrays.copyOf(stateBuckets, stateBuckets.length*2);
                    bucketSizes = Arrays.copyOf(bucketSizes, bucketSizes.length*2);
                }
                
                // Optimisation: Try transfer old cleared bucket over. (reuse buckets to avoid reconstruction when possible)
                if (nextUnreusedBucket < nextBucket) {
                    stateBuckets[numBuckets] = stateBuckets[nextUnreusedBucket];
                    stateBuckets[nextUnreusedBucket] = null; // Safeguard. can remove.
                    ++nextUnreusedBucket;
                } else {
                    stateBuckets[numBuckets] = new ScanInterval[11];
                }
                
                bucketSizes[numBuckets] = 0;
                ++numBuckets;
            }
            
            // Add to bucket.
            if (bucketSizes[bucketNo] >= stateBuckets[bucketNo].length) {
                stateBuckets[bucketNo] = Arrays.copyOf(stateBuckets[bucketNo], stateBuckets[bucketNo].length*2);
            }
            stateBuckets[bucketNo][bucketSizes[bucketNo]] = successor;
            ++bucketSizes[bucketNo];
        }
    }
    ...
    protected List<SnapshotItem> computeSearchSnapshot() {
        ArrayList<SnapshotItem> list = new ArrayList<>(numQueueStates);

        for (int i=0;i<numQueueStates;++i) {
            ScanInterval in = statesQueue[i];
        ...
        for (int b=nextBucket;b<numBuckets;++b) { // try to print the states that are going to be popped next too.
            for (int i=0;i<bucketSizes[b];++i) {
                ScanInterval in = stateBuckets[b][i];
                // y, xLn, xLd, xRn, xRd, px, py
                if (in == null) continue;
