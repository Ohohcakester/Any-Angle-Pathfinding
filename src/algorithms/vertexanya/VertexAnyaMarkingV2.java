...
public class VertexAnyaMarkingV2 extends PathFindingAlgorithm {
    ...
    public VertexAnyaMarkingV2(GridGraph graph, int sx, int sy, int ex, int ey) {
    ...
    private final void generateStartSuccessors() {
        ...
        if (!topRightOfBlocked || !bottomRightOfBlocked) {
            int x = leftAnyExtentMarking(sx, sy, 0, start);
            int y = sy;
            if (y == ey && x <= ex && ex <= sx) {
        ...
        if (!topLeftOfBlocked || !bottomLeftOfBlocked) {
            int x = rightAnyExtentMarking(sx, sy, 0, start);
            int y = sy;
            if (y == ey && sx <= ex && ex <= x) {
                tryRelax(start, sx, sy, ex, ey);
    ...
    private final void generateSuccessors(int current, int parent) {
        ...
        if (leftwardsSearch) {
            ...
            int x = leftAnyExtentMarking(baseX, baseY, memory.distance(current), current);
            int y = baseY;
        ...
        if (rightwardsSearch) {
            ...
            int x = rightAnyExtentMarking(baseX, baseY, memory.distance(current), current);
            int y = baseY;
    ...
    private final int leftAnyExtentMarking(int xL, int y, float baseDistance, int baseIndex) {
        baseDistance += EPSILON;
        boolean trVal = graph.topRightOfBlockedTile(xL, y);
        boolean brVal = graph.bottomRightOfBlockedTile(xL, y);
        do {
            --xL;
            ...
            int index = graph.toOneDimIndex(xL, y);
            if (baseDistance < memory.distance(index)) {
                memory.setDistance(index, baseDistance);
                memory.setParent(index, baseIndex);
            }
        } while (xL > 0 && (graph.topRightOfBlockedTile(xL, y) == trVal) && (graph.bottomRightOfBlockedTile(xL, y) == brVal));
        return xL;
    }
    ...
    private final int rightAnyExtentMarking(int xR, int y, float baseDistance, int baseIndex) {
        baseDistance += EPSILON;
        boolean tlVal = graph.topLeftOfBlockedTile(xR, y);
        boolean blVal = graph.bottomLeftOfBlockedTile(xR, y);
        do {
            ...
            int index = graph.toOneDimIndex(xR, y);
            if (baseDistance < memory.distance(index)) {
                memory.setDistance(index, baseDistance);
                memory.setParent(index, baseIndex);
            }
    ...
    private final void generateAndSplitIntervals(int checkY, int newY, int baseX, int baseY, Fraction leftBound, Fraction rightBound, boolean leftInclusive, boolean rightInclusive) {
        ...
        while(true) {
            //RightDownExtent. it's actually rightDownExtents for exploreDownwards. (thus we use checkY = currY - 2)
            if (rightBound.isLessThanOrEqual(leftFloor)) break;
            
            int right = leftFloor;
            boolean withinRightBound = true;
            boolean val = graph.topLeftOfBlockedTile(right, checkY);
            do {
                int index = graph.toOneDimIndex(right, newY);
                float distance = baseDistance + graph.distance(baseX, baseY, right, newY) + EPSILON;
                if (left.isLessThanOrEqual(right) && distance < memory.distance(index)) {
                    memory.setDistance(index, distance);
                    memory.setParent(index, baseIndex);
                }
                
                ++right;
                if (rightBound.isLessThanOrEqual(right)) withinRightBound = false;
            } while (withinRightBound && graph.topLeftOfBlockedTile(right, checkY) == val);

            if (!withinRightBound) break; // right < rightBound
        ...
        if (rightBound.isWholeNumber()) {
            int _x = rightBound.n;
            int index = graph.toOneDimIndex(_x, newY);
            float distance = baseDistance + graph.distance(baseX, baseY, _x, newY) + EPSILON;
            if (distance < memory.distance(index)) {
                memory.setDistance(index, distance);
                memory.setParent(index, baseIndex);
            }
        }
