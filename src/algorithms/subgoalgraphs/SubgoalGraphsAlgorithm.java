...
public class SubgoalGraphsAlgorithm extends PathFindingAlgorithm {
    ...
    public SubgoalGraphsAlgorithm(GridGraph graph, int sx, int sy, int ex, int ey) {
    ...
    private final void computeOctilePath() {
        ...
        int maxLength = pathNumberOfHops()*2;
        ...
        while (current != startIndex) {
            ...
            int currX = subgoalGraph.xPositions[current];
            int currY = subgoalGraph.yPositions[current];
            int parX = subgoalGraph.xPositions[parent];
            int parY = subgoalGraph.yPositions[parent];
    
            int absdx = Math.abs(parX - currX);
            int absdy = Math.abs(parY - currY);
    
            if (absdy > absdx) {
                if (absdx != 0) {
                    int midX = currX;
                    int midY = (parY > currY) ? (parY-absdx) : (parY+absdx);
                    
                    tempPath[length++] = new int[]{midX, midY};
                }
            } else if (absdy < absdx) {
                if (absdy != 0) {
                    int midX = (parX > currX) ? (parX-absdy) : (parX+absdy);
                    int midY = currY;
            ...
            tempPath[length++] = new int[]{parX, parY};
            current = parent;
        }
