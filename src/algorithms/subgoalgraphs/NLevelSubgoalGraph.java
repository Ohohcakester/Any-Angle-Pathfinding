...
public class NLevelSubgoalGraph {
    //public static Runnable snapshotFunction;
    
    private static final float EPSILON = 0.00001f;
    private static NLevelSubgoalGraph storedGraph;
    ...
    private int[] queue;
    public HReachabilityChecker checker;
    
    private int originalSize;
    public int startIndex;
    public int endIndex;
    
    private NLevelSubgoalGraph(int numLevels) {
    ...
    public static final NLevelSubgoalGraph initialiseNew(GridGraph graph, int numLevels) {
        if (NLevelSubgoalGraph.graph == graph && NLevelSubgoalGraph.storedGraph.maxLevel == numLevels) {
            return storedGraph;
        }
        NLevelSubgoalGraph.graph = graph;
        NLevelSubgoalGraph subgoalGraph = NLevelSubgoalGraph.storedGraph = new NLevelSubgoalGraph(numLevels);
    ...
    public final void initialiseGraph() {
        ...
        computeClearances();
        checker = new HReachabilityChecker(xPositions, yPositions, graph);
            ...
            for (int curr=0;curr<size;++curr) {
                ...
                for (int i=0;i<numNeighbours;++i) {
                    ...
                    for (int j=i+1;j<numNeighbours;++j) {
                        ...
                        if (!checker.isHReachable(n1, n2) && !spChecker.hasShorterGlobalPath(n1, n2, curr)) {
                            // necessary.
                            necessary = true;
                            break;
                        }
    ...
    public static final void clearMemory() {
        NLevelSubgoalGraph.graph = null;
        NLevelSubgoalGraph.storedGraph = null;
        System.gc();
    }
}
